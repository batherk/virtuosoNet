...
import settings
from path_functions import get_current_directory_path as dpath
...
def load_stat_file(
    data_name,
    num_prime_param=settings.NUM_PRIME_PARAM,
    num_trill_param=settings.NUM_TRILL_PARAM,
    num_tempo_param=settings.NUM_TEMPO_PARAM,
    num_output=settings.NUM_OUTPUT,
    training_loss="MSE",
):
    with open(dpath() + data_name + "_stat.dat", "rb") as f:
...
def load_params(model_type):
    return param.load_parameters(dpath() + model_type + "_param")


def initialize_model(model_type, net_param, device=settings.DEVICE):
...
def edges_to_matrix(
    ...
    num_notes,
    model,
    graph_keys=settings.GRAPH_KEYS,
    n_edge_type=settings.N_EDGE_TYPE,
):
...
def edges_to_sparse_tensor(edges, graph_keys=settings.GRAPH_KEYS):
    num_keywords = len(graph_keys)
    edge_list = []
    edge_type_list = []
...
def scale_model_prediction_to_original(
    prediction, means, stds, bins, loss_type=settings.LOSS_TYPE
):
...
def load_file_and_generate_performance(
    ...
    num_input=settings.NUM_INPUT,
    HIER_model=settings.HIER_MODEL,
...
def load_file_and_encode_style(
    ...
    num_input=settings.NUM_INPUT,
    num_output=settings.NUM_OUTPUT,
    hier_meas=settings.HIER_MEAS,
    hier_beat=settings.HIER_BEAT,
):
    absolute_path = dpath() + path
...
def encode_performance_style_vector(
    input, input_y, edges, note_locations, model, device=settings.DEVICE
):
    with torch.no_grad():
...
def encode_all_emotionNet_data(path_list, style_keywords, in_HIER=settings.IN_HIER):
...
def handle_data_in_tensor(
    ...
    device=settings.DEVICE,
    num_trill_param=settings.NUM_TRILL_PARAM,
    is_hierarchy=settings.HIERARCHY,
    is_trill=settings.TRILL,
    hierarchy_test=settings.HIERARCHY_TEST,
):
    x = torch.Tensor(x)
    y = torch.Tensor(y)
    if hier_meas:
